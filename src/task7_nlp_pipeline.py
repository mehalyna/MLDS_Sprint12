...
def analyze_documents(
    ...
    if len(documents) < 2:
        raise ValueError("At least 2 documents are required for analysis")
    ...
    if not isinstance(ngram_range, tuple) or len(ngram_range) != 2:
        raise ValueError("ngram_range must be a tuple of two integers")
    if ngram_range[0] > ngram_range[1] or ngram_range[0] < 1:
    ...
    for doc in documents:
        ...
        if tokens:  # Only process non-empty token lists
            ...
            processed_tokens = result['lemmatized' if use_lemmatization else 'stemmed']
    ...
    non_empty_docs = [doc for doc in preprocessed_docs if doc]
    
    if non_empty_docs:
        word2vec_model = train_word2vec(non_empty_docs, vector_size=100, window=5)
    ...
    bigrams = generate_ngrams(full_text, 2) if len(full_text.split()) >= 2 else []
    trigrams = generate_ngrams(full_text, 3) if len(full_text.split()) >= 3 else []
    
    bigram_counts = count_ngrams(bigrams) if bigrams else {}
    trigram_counts = count_ngrams(trigrams) if trigrams else {}
    ...
    bigram_diversity = len(bigram_counts) / len(bigrams) if bigrams else 0.0
...
def find_similar_documents(
    ...
    similarities = similarity_matrix.iloc[doc_index].to_dict()
    
    # Remove self-similarity
    doc_key = f"Doc_{doc_index}"
    if doc_key in similarities:
        del similarities[doc_key]
...
def get_document_keywords(
    ...
    doc_features = feature_matrix.iloc[doc_index]
    
    # Sort by score (descending) and get top N
    top_features = doc_features.nlargest(top_n)
